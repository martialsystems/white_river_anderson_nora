...
"""Fail closed: gage lag, not inundation, not feet."""
...
from upq.errors import ClaimBanError
...
_BANS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ...
    ("lag_wet", re.compile(r"\blag[- ](?:scatter|correlation|map) is (?:a |the )?(?:wet mask|inundation)\b", re.I)),
    ("flood_warning", re.compile(r"\bflood warning\b|\bemergency forecast\b", re.I)),
    ("indy_pred", re.compile(r"\bIndianapolis (?:is|as) a Nora predictor\b", re.I)),
    ("feet_invert", re.compile(r"\binvert(?:ed|ing)? Nora Q to feet\b", re.I)),
    ("nwm_repull", re.compile(r"\bre-pull(?:ed|ing)? NWM\b", re.I)),
