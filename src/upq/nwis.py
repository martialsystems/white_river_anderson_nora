...
"""NWIS daily 00060. Indianapolis is fetched for diagnostic only."""
...
from upq.config import ANDERSON_ID, INDY_ID, NORA_ID, NWIS_DV_URL
from upq.errors import FetchError
from upq.http import get_json
...
def fetch_three(*, start: date, end: date, get_json_fn=None) -> dict[str, dict[np.datetime64, float]]:
    return {
        ANDERSON_ID: fetch_q(site=ANDERSON_ID, start=start, end=end, get_json_fn=get_json_fn),
        NORA_ID: fetch_q(site=NORA_ID, start=start, end=end, get_json_fn=get_json_fn),
        INDY_ID: fetch_q(site=INDY_ID, start=start, end=end, get_json_fn=get_json_fn),
    }
